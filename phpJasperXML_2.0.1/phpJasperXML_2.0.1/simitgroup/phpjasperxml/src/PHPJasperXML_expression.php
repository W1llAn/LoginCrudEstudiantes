    ...
    protected function parseExpression(string $expression, int $addrowqty = 0, string $evaluationTime = ''): mixed
{
    ...
    foreach ($fieldnames as $f => $fieldname) {
        if (isset($this->fields[$fieldname])) {
            ...
            // Manejar el caso en el que el campo no está definido
            // Por ejemplo, podrías asignar un valor predeterminado o lanzar una excepción
        }
    }
    ...
    foreach ($varnames as $v => $varname) {
        if (isset($this->variables[$varname])) {
            ...
            // Manejar el caso en el que la variable no está definida
        }
    }

    foreach ($paranames as $p => $paraname) {
        if (isset($this->parameters[$paraname])) {
            ...
            // Manejar el caso en el que el parámetro no está definido
        }
    }
